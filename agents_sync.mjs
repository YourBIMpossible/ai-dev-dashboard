...
import { writeFileSync, readFileSync, readdirSync, createReadStream, statSync } from "node:fs";
...
const agents = {
  // Wall-clock time, not nowMs (latest session activity): nowMs is a DATA timestamp
  // (used below for activeToday/active7d windows) and on a day with zero new Claude
  // Code activity it wouldn't advance at all, defeating the point of always rewriting
  // this stamp (2026-09-07).
...
// Always rewrite, even when nothing but `generated` changed: this file's timestamp
// IS the dashboard health-strip's staleness signal, and skipping the write on an
// idle day (no new session activity) is exactly what let this chip go 48 days
// stale while the underlying data was still correct. Whether a stamp-only change
// is worth a git commit is Refresh-Dashboard.ps1 / git's call (git diff sees the
// changed timestamp and stages it, which is the desired outcome here), not this
// script's (2026-09-07).
const banner =
