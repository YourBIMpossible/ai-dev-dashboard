    ...
    /* PROJECT:bimpossible:START */
    {
      ...
      progress: {
        ...
        phases: [
          ...
          { name: "P13.1 Write Engine — Typed Values + Type Params (Phase 13 sub-increment)", pct: 50, note: "PARTIAL — Placed 2026-08-04 (owner decision): ships as a Phase 13 sub-increment, not its own phase — resolves the design brief's final open question. Extends the T4 apply engine beyond String/instance: typed values now, family-TYPE targeting later. Design brief `2026-07-25__WriteEngine_TypedValues_and_TypeParams_DesignBrief_PROPOSAL.md` (all 5 §5 questions owner-ratified 2026-07-25); build plan + corrected live-smoke script + open decisions: `design-docs/2026-07-26__write-engine-increment1_typed-values-instance_build-plan.md` (`099080c`). Increment 1 (non-string, INSTANCE-scoped values) BUILT + review-clean 2026-07-27, deliberately unmerged — backend draft [PR #232](https://github.com/YourBIMpossible/BIMpossible/pull/232) (`feat/write-engine-increment1` @ `449b072`): `staged_change.value_type` ('string'/'integer'/'double') + `unit` with paired CHECK constraints (migration `9c9a59a39d64`), service validation 422-before-constraint, wired through `/save` + `/stage`; Verify-Local-CI GREEN (DB lane 3148 / pure 2128), `alembic check` clean. Add-Ins draft [PR #49](https://github.com/YourBIMpossible/BIMpossible-AddIns/pull/49) (@ `68673c8`): ApplyOne writes Integer + unit-aware Double via one shared `ConvertDisplayToInternal` off the LIVE parameter's `GetUnitTypeId()` (exactly one `ConvertToInternalUnits` call tree-wide — reuses the proven write spine, nothing reimplemented); `SkipUnsupportedStorage` renamed `SkipElementIdNotYetSupported` (wire string unchanged) + new `SkipValueTypeMismatch` bucket; group-separated numerics (`\"1 234.56\"`) refused rather than silently written 1000× off; 1470 tests, both TFMs. ElementId: detected + reported by name, deliberately unimplemented (Increment 4, owner ruling 07-25); Increment 2 (type-param targeting, String-only) and Increment 3 (combined) unbuilt — each gated on the prior increment's live smoke per the brief's §6 sequencing. Closing gate for Increment 1: live Revit smoke — script CORRECTED 2026-07-27 after final review: use a millimetre length parameter, NOT `usd` (currency's factor-1.0 conversion proves nothing whether the conversion is right, inverted, or absent); plus empty-parameter, malformed-value, and space-grouped cases — and 3 owner decisions recorded in the build plan's \"Known, deliberately UNRESOLVED\" section (staged `unit` is fetched but never read — veto guard?; unset numerics read as `0` not blank, so first-time writes may drift-skip; `BuildSummary` bucket-exhaustiveness unenforced). Merge order matters: backend first or together (the new `skip_value_type_mismatch` wire status 422s against an old backend, losing that set's audit rows); deploy slots must build from Add-Ins main (carries RE-1 fix #46) + this branch." },
          {
      ...
      activity: [9,3,20,16,9,8,2,0,0,0,0,0,0,6],
      lastActivity: {
        date: "2026-08-04",
        summary: "fix(synthetic-audit): land the 2026-07-26/27 run hardening that was floating uncommitted (#241) (2eb20c3)"
      },
