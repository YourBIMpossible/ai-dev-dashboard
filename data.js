...
window.DASHBOARD_DATA = {
  ...
  projects: [
    ...
    /* PROJECT:addins:START */
    {
      ...
      focus: "Write Engine Increment 1 (Add-Ins half) BUILT + review-clean on draft PR #49 (feat/write-engine-increment1 @ 68673c8): ApplyOne now writes Integer and unit-aware Double instance parameters — live-unit conversion through one shared ConvertDisplayToInternal (single ConvertToInternalUnits call tree-wide), group-separated numerics refused instead of silently written 1000x off, ElementId detected + reported by name and deliberately unimplemented (Increment 4). 1470 tests, both TFMs. Merge order: backend PR #232 first or together; then build slots from main+branch and run the corrected live smoke (mm length parameter, not usd — usd's 1.0 factor proves nothing). — Family Fixer's ribbon button is merged but not yet live-clicked, and go_single_panel — its one destructive operation — has never been exercised live; that's the real remaining risk, not a merge gate. Confirm the abandoned phase15a-pane rebase line is safe to lose. Hash-verify the %APPDATA% deploy slot against current HEAD (1366569) — last explicit slot-ledger entry predates PR #40's deploy-script fix.",
      progress: {
        label: "Tracks",
        phases: [
