...
window.DASHBOARD_DATA = {
  ...
  projects: [
    /* PROJECT:bimpossible:START */
    {
      ...
      focus: "RE-1 (High, carried — 07-27 audit): EventDispatcher.Execute() still needs cancellation or ID-matched removal when Raise() is rejected. Newly confirmed dormant in every shipped build (BIMPOSSIBLE_ENABLE_PIPE unset everywhere) but zero test coverage exists — fix before the pipe path is ever re-enabled, not after. Two new Medium security gaps from this week's real new surface (wizard OAuth-consent identity binding; a live firm-membership-resolution divergence that already caused one real incident) are the next-most-actionable items. Separately: close the branch-protection gap (enforce_admins=false — every required check is a signal, not a gate, on the direct-to-main path). Two trivial fixes (ARCH-ENGINE-BYPASS, CQ-WSR8-DOC) are stuck unfixed for a 2nd cycle purely because the scheduled audit's sandbox has no Docker/PowerShell to verify CI.",
      progress: {
      ...
      activity: [10,16,5,0,0,0,2,36,9,3,20,16,9,0],
      lastActivity: {
        date: "2026-07-26",
        summary: "docs(reference): land Winchester demo shared-parameter reference doc (9ea73c3)"
      },
      ...
      nextActions: ["Fix RE-1 (High, carried): add a cancel/ID-match-remove step when Raise() is rejected — a proving test is buildable today with no infra changes (EventDispatcher/PipeServer compile unconditionally)","SEC-WIZ-CONSENT-1 (Medium, new): add an identity-match check (JWKS/userinfo sub vs. recorded autodesk_user_id) before storing the wizard's elevated-consent token","SEC-MEMBERSHIP-1 (Medium, new): audit every get_firm_id() call site before onboarding a 2nd firm_domain — make it agree with require_active_membership()'s strict 403","Close the branch-protection gap: enforce_admins=false means required checks (incl. the Alembic guard) are a signal, not a gate, on the direct-to-main push path","Land ARCH-ENGINE-BYPASS + CQ-WSR8-DOC (both S-effort, trivial) from a Windows/Docker-capable session — 2 straight audit cycles stuck purely on the sandboxed Auto-Fix Pass's CI-verification gap","True-prod deploy (runbook 06-12): alembic upgrade head, set BIMPOSSIBLE_ADMIN_ENABLED=1 + BIMPOSSIBLE_ADMIN_SECRET, DELETE stale done-rows in relationship_prewarm_jobs per model"],
      pendingDecisions: [
      ...
      blockers: [],
      reminders: ["Branch protection has enforce_admins=false and no required-PR-review, and Push-And-Verify.ps1 pushes direct to main as admin — required checks (incl. the Alembic single-head guard) are a SIGNAL, not a gate, on the real push path.","The scheduled weekly-audit's Auto-Fix Pass has gone 3 consecutive cycles (07-13, 07-20, 07-27) unable to apply even one-line, zero-risk fixes because its sandbox has no Docker/PowerShell — worth deciding whether to run it from elsewhere or re-scope it report-only.","Phase 3.10a's _most_common_phase is single-phase-only by design (self-documented); needs real per-element Phase data or an explicit picker before multi-phase projects rely on it","reportlab not installed → aec/exports.py PDF routes inert (CSV works); add before Phase C /account/export/*.pdf","My Account shows graceful 'not linked' state until founder's Autodesk user gets an active user_firm_memberships row"],
      links: [
      ...
      audit: {
        lastRun: "2026-07-27",
        runType: "Weekly full audit — scheduled, autonomous, read-only through findings-gathering, user not present. 3 parallel lens sub-agents (Backend/data/ops, Frontend, Revit-relay/add-in+CI+ops+tests) plus direct re-verification of load-bearing claims — including one sub-agent's own Security finding checked and DISPROVEN by the consolidating pass (it searched commit messages and the build spec, not decision-log/, where this project actually records write-operation sign-offs). Auto-Fix Pass applied 0 fixes for the 3rd consecutive week — this sandbox still has no Docker/PowerShell to verify CI, so even trivial comment-only fixes go to human-review rather than being applied unverified.",
        cadence: "weekly Mon 6am + on-demand",
        counts: { critical: 0, high: 1, medium: 8, low: 6, info: 6 },
        closedLastRun: 0,
        trend: "flat",
        reportPath: "F:\\AI-Dev\\BIMpossible_Workspace\\02_Reference\\Audit and Scan Info\\weekly-full-audit_2026-07-27.md",
        reportFile: "bimpossible/weekly-full-audit_2026-07-27.md",
        ledgerPath: "F:\\AI-Dev\\BIMpossible_Workspace\\02_Reference\\_audit-runs.md",
        open: [
          { id: "RE-1", sev: "high", title: "EventDispatcher.Execute() drains its whole pending-request queue FIFO; a request whose Raise() was REJECTED stays queued and its DispatchCommand (incl. write_instance_parameter) executes for real, silently, on the next unrelated successful raise. Mechanism reconfirmed byte-for-byte unchanged this cycle. NEW this run: independently confirmed BIMPOSSIBLE_ENABLE_PIPE is unset in every shipped/CI/deploy path — the bug is dormant in production today, not eliminated — but EventDispatcher/PipeServer are referenced in zero test files and both classes compile unconditionally, so it's one flag-flip from live with nothing to catch it. A proving test is buildable today with no infra changes.", where: "Add-Ins/BIMpossible.RevitLink/EventDispatcher.cs:41-55 · PipeServer.cs:156-171" },
          { id: "SEC-WIZ-CONSENT-1", sev: "medium", title: "Wizard elevated-consent OAuth callback never re-verifies the Autodesk identity completing the code exchange matches the identity that started consent — unlike the primary login flow's JWKS/userinfo check. A different Autodesk account completing the browser round trip gets its token stored under the ORIGINAL recorded identity; the write later executes with the wrong account's real permissions while every audit trail still shows the original user. Genuinely new — this code landed this week (the wizard's Forma rework). No proving test exists today.", where: "backend/auth_router.py:433-461 · backend/wizard/consent.py:57-91 · backend/wizard/router.py:220-246" },
          { id: "SEC-MEMBERSHIP-1", sev: "medium", title: "get_firm_id()'s silent static-firm fallback vs. require_active_membership()'s strict 403 now have a LIVE divergence path for the first time, since 7dc41c8 wired real OAuth-login membership-linking into production. The moment a 2nd firm_domain is registered, routers resolving via get_firm_id() would silently bucket an unlinked user into the shared static firm rather than deny. Already caused one real incident: Phase 13's first live Change Set Save 403'd for exactly this reason (hand-patched; 7dc41c8/PR #227 is documented as \"the permanent fix\" — this finding is that the fix isn't yet applied at every call site).", where: "backend/aec/identity.py:39-45,63-81 · backend/aec/membership.py:71-77" },
          { id: "SEC-NPMALERT-1", sev: "medium", title: "(= WSR13) 2 open HIGH-labeled Dependabot alerts (brace-expansion, glob, both dev-tooling transitives in frontend/package-lock.json) surfaced by WSR9a's 2026-07-25 fix and explicitly handed to \"the next weekly audit\" — still untriaged; confirmed via git log that nothing has touched package-lock.json/package.json since. This run is that handoff, logged formally now.", where: "frontend/package-lock.json" },
          { id: "SEC-ADDINS-NOSCAN-1", sev: "medium", title: "(= WSR14) The Add-Ins repo's own CI (ci.yml + dashboard-sync.yml) has no gitleaks/Trivy/Semgrep/pip-audit-equivalent job, unlike the main repo's hard-gated security-scan.yml — a gap that matters more than cosmetically now that this repo carries real session/pairing-token code (Phase 15a PaneTokenStore/PaneSessionHandler).", where: "Add-Ins/.github/workflows/ci.yml (whole file); no security-scan.yml present" },
          { id: "SEC-3", sev: "medium", title: "wizard/gate.py validates hub_id by allowlist membership, not real per-user Autodesk hub membership — latent IDOR once multi-tenant; today produces orphaned/misattributed rows at worst, no APS write at plan time. Carried, unchanged, accepted per the Verification Checklist; the Forma rework didn't touch this logic.", where: "backend/wizard/gate.py:49-66" },
          { id: "RE-2", sev: "medium", title: "(= OPS-C1) Single-pipe-instance (maxNumberOfServerInstances:1) vs. the relay's 4-worker thread pool — the pool exists for wedge-isolation, not real concurrency. Caps Revit-link throughput by design; needs RE-1 fixed first before raising the instance cap. Unchanged, accepted current scope.", where: "PipeServer.cs:82,97 · relay.py:107-108" },
          { id: "ARCH-NEW-2", sev: "medium", title: "aec/router.py re-measured at exactly 2,828 lines — byte-identical to last week, no regression. Maintainability drag only; domain split at next major touch. Accepted-deferred, unchanged.", where: "backend/aec/router.py" },
          { id: "FE-3", sev: "medium", title: "model/page.tsx god-component now 3,450 LOC (was 3,327 last week, +3.7% — much slower growth than the prior +17% jump). Deliberately deferred pending the 5k-element benchmark trigger, not an oversight.", where: "frontend/app/project/[id]/model/page.tsx" },
          { id: "ARCH-ENGINE-BYPASS", sev: "low", title: "Read-only drift-check baseline read still constructs RevitNativeAdapter() directly instead of via get_engine() — STILL UNFIXED for a 2nd cycle. S-effort, zero risk, comment-adjacent fix, but the sandboxed Auto-Fix Pass cannot verify CI here (no Docker/PowerShell) so it keeps getting routed to human-review instead of applied.", where: "backend/aec/assistant_write_tools.py:484-487" },
          { id: "CQ-WSR8-DOC", sev: "low", title: "Stale docstrings on the write-gate path — STILL UNFIXED for a 2nd cycle, and the condition got objectively WORSE: assistant_write_tools.py:457-464 now names the wrong gate (check_firm_view_editor) in one paragraph and the correct gate (check_firm_model_editor) in the very next, internally self-contradictory rather than merely outdated. Code is correct and tested; only the comments are wrong.", where: "backend/aec/assistant_parameter_writes.py:13-23 · assistant_write_tools.py:457-464" },
          { id: "RE-WIZ-POLL-1", sev: "low", title: "Wizard reclaim-sweep's stale-threshold comment understates the real worst-case poll duration by ~2x after the Forma rework (240s possible vs. documented \"seconds to a couple minutes\"). 900s threshold still holds ~3-4x margin today — not a live bug, but a future poll-budget bump could erode the margin unnoticed.", where: "backend/wizard/reclaim_worker.py:19-24" },
          { id: "FE-PARAMDLG-1", sev: "low", title: "EditParameterDialog and SyncConflictModal in model/page.tsx are statically imported despite being genuinely open-on-demand — every sibling modal on the same route correctly uses dynamic(...,{ssr:false}) per the CLAUDE.md 207KB perf-budget rule; these two were missed. Pre-existing since 2026-06-01, first itemized this cycle, not a regression.", where: "app/project/[id]/model/page.tsx:67,69,3381,3392" },
          { id: "FE-A11Y-1", sev: "low", title: "New ChangeSetStatusPill popover has no aria-expanded/aria-haspopup/role/Escape-to-close, inconsistent with the shared useModalDialog pattern already adopted by 11 other overlays. Read-only display, no destructive action — low blast radius.", where: "app/components/ChangeSetStatusPill.tsx:80-102" },
          { id: "SEC-ADDINS-DEADCITE-1", sev: "low", title: "EventDispatcher.cs's security-refusal comment cites a specific audit doc (audits/2026-07-25__incremental-audit-since-521e612.md, H-8) as its sole authority — that file doesn't exist anywhere in the Add-Ins repo. Not a live vulnerability, but a future engineer could remove the refusal thinking the cited authority doesn't hold up.", where: "Add-Ins/BIMpossible.RevitLink/EventDispatcher.cs:115,125" },
          { id: "SEC-WIZ-APPROVAL-1", sev: "info", title: "DISPROVEN this run — kept visible for the record, not a real finding. A sub-agent claimed the Forma Hub Admin API wizard write-path rework had no dated, per-operation human sign-off. FALSE POSITIVE: decision-log/2026-07-22__phase8-project-setup-wizard-go-live.md documents the founder's dated, scope-matched approval, corroborated independently by BIMpossible_PHASE-STATUS.md row 8. The sub-agent searched commit messages and the build spec, not decision-log/ — logged as WSR10 with the disproof so the search-scope lesson isn't lost.", where: "decision-log/2026-07-22__phase8-project-setup-wizard-go-live.md" },
          { id: "SEC-cloud-1", sev: "info", title: "IMPROVED this cycle: backend/aps/cloud_ids.py now exists (didn't last week) and is more conservative than the original finding assumed — an unrecognized region returns None explicitly, never a guessed default. EMEA region-inference risk remains latent but unexercised (no EMEA hub onboarded).", where: "backend/aps/cloud_ids.py:18-24,64-77" },
          { id: "ARCH-ADDINS-TEST-COUNT", sev: "info", title: "Recounted at 895 (771 Fact + 124 Theory) vs. 634 reported last week. Reconciled as real, feature-driven growth (31 new test files: Assistant/pane, FamilyFixer, T4/Apply), confirmed by re-running the identical count against last week's actual commit (a661924), which gives 645 — consistent with 634 within normal methodology noise, not measurement drift.", where: "BIMpossible.RevitLink.Tests/**" },
          { id: "CQ-WIZ-LEGACY-1", sev: "info", title: "planner.py correctly documents two now-legacy plan-item types kept only for pre-rework persisted rows. Flagged only so a future cleanup checks for lingering planning-status rows first.", where: "backend/wizard/planner.py:33-34" },
          { id: "FE-BASELINE-1", sev: "info", title: "Last week's \"no raw-fetch bypass beyond initViewer.ts:78\" framing is stale wording, not a regression — two more pre-existing, deliberately-designed, tested exceptions exist (lib/api/persistence.ts, lib/adminApi.ts). Recommend updating the checklist's wording rather than treating as new risk.", where: "app/lib/api/persistence.ts · app/lib/adminApi.ts" },
          { id: "INFO-UNTRACKED-PS1", sev: "info", title: "Invoke-SyntheticConcurrencyAudit.ps1 (root, drives real Docker teardown/mutation) is untracked in git. Not a security issue on its own, but an untracked script that can mutate Docker state has no change history or review trail.", where: "BIMpossible/Invoke-SyntheticConcurrencyAudit.ps1" }
        ],
        history: [
          { date: "2026-07-27", type: "Weekly full audit (scheduled, autonomous, 3 lens sub-agents) — read-only through findings-gathering; Auto-Fix Pass write-enabled but applied 0 fixes", scope: "Main repo HEAD 569bcb8 (47 commits since 2026-07-20's 29e96da). Add-Ins repo HEAD cc4adc3. Largest new surface: Phase 8 Wizard's Forma Hub Admin API rework (first-ever live Autodesk write), Phase 13 Change Sets T0-T4/Task 6, Phase 15a Revit-pane pairing, OAuth firm auto-linking, an Auth.js critical-CVE patch.", result: "Severity flat vs last week (0/1/8/8/6 vs 0/1/~8/~10/~3) — 0 findings resolved in code this cycle. One sub-agent's Security claim (SEC-WIZ-APPROVAL-1, no dated write-approval) was checked by the consolidating pass and DISPROVEN — the sign-off exists in decision-log/, just outside that sub-agent's search scope. Real new production surface (wizard's first live Autodesk write, Change Set staging lifecycle, live Revit-pane pairing) assessed as competently engineered with real defense-in-depth; produced 2 new Medium security gaps (wizard OAuth-consent identity binding not re-verified across the token exchange; a live firm-membership-resolution divergence that already caused one real incident, hand-patched, permanent fix not yet applied everywhere). Two trivial S-effort fixes (ARCH-ENGINE-BYPASS, CQ-WSR8-DOC) are now unfixed for a 2ND CONSECUTIVE CYCLE purely because this sandbox has no Docker/PowerShell to verify CI before applying even a comment-only change — the 3rd straight week with this exact operational gap; escalated recommendation to run the Auto-Fix Pass from a Windows/Docker-capable session or re-scope it report-only.", report: "weekly-full-audit_2026-07-27.md" },
    ...
    /* PROJECT:addins:START */
    {
      ...
      activity: [16,19,4,0,0,0,1,12,2,0,6,11,2,0],
      lastActivity: {
        date: "2026-07-26",
    ...
    /* PROJECT:families:START */
    {
      ...
      activity: [0,0,3,0,0,0,0,1,3,2,2,0,1,0],
      lastActivity: {
        date: "2026-07-26",
