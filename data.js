...
window.DASHBOARD_DATA = {
  ...
  projects: [
    /* PROJECT:bimpossible:START */
    {
      ...
      nextActions: ["Decide the Auto-Fix Pass's fate: give the scheduled runner a Windows/Docker lane, or formally re-scope that stage report-only — 4 cycles, zero fixes applied, now preflight-honest but still blocked","Close the branch-protection gap: enforce_admins=false means required checks (incl. the Alembic single-head guard) are a signal, not a gate, on the direct-to-main push path","Prove the new Add-Ins gitleaks gate actually fires — it was validated only against a clean 442-commit history, which can't distinguish a working gate from a broken one","True-prod deploy (runbook 06-12): alembic upgrade head, set BIMPOSSIBLE_ADMIN_ENABLED=1 + BIMPOSSIBLE_ADMIN_SECRET, DELETE stale done-rows in relationship_prewarm_jobs per model","Re-pair the Revit pane to verify B-1's live /account/me → role:admin claim and whether the project dropdown populates (the pane token used for overnight checks was revoked by sign-out, by design)","Visually confirm A-1 (#246, account link in headers) in a browser — merged on green CI + typecheck but never eyeballed","B-4: active Revit sessions list + revoke — the one item from the overnight backlog that's still genuinely not done, not deferred"],
      ...
      audit: {
        lastRun: "2026-08-04",
        runType: "Weekly full audit (autonomous, 3 lens sub-agents, read-only) — followed the same day by PR #239, which closed every one of the 6 new findings it raised, hours after the report was written. Counts below reflect that close-out, not the report's own summary line, which was already stale by ~2 hours. Auto-Fix Pass itself was preflight-BLOCKED for a 4th cycle (no PowerShell/Docker in the scheduled runner), so the fixes came from a human session, not the pass.",
        cadence: "weekly Sun 11:45pm + incremental Sun/Tue + on-demand",
        counts: { critical: 0, high: 0, medium: 4, low: 3, info: 3 },
        closedLastRun: 6,
        trend: "improving",
        reportPath: "F:\\AI-Dev\\BIMpossible_Workspace\\02_Reference\\Audit and Scan Info\\weekly-full-audit_2026-08-04.md",
        reportFile: "bimpossible/weekly-full-audit_2026-08-04.md",
        ledgerPath: "F:\\AI-Dev\\BIMpossible_Workspace\\02_Reference\\_audit-runs.md",
        open: [
          { id: "SEC-3", sev: "medium", title: "wizard/gate.py checks a hub_id allowlist instead of real per-user Autodesk hub membership — a latent IDOR once the product is multi-tenant. Today it produces only orphaned/misattributed planning rows, with no APS write at plan time. CARRIED-OPEN, ACCEPTED per the Verification Checklist; code byte-identical again this cycle.", where: "backend/wizard/gate.py:49-66" },
          { id: "RE-2", sev: "medium", title: "(= OPS-C1) A single pipe instance versus a 4-worker relay thread pool — the pool exists for wedge-isolation, not real concurrency, so Revit-link throughput is capped at one operation at a time by design. CARRIED-OPEN, accepted current scope. Unaffected by RE-1's fix (different mechanism), but raising the instance cap is now unblocked by it.", where: "PipeServer.cs:82,97 · revit-relay/relay.py:107" },
          { id: "ARCH-NEW-2", sev: "medium", title: "aec/router.py re-measured at exactly 2,828 lines — byte-identical for a 2nd straight week. Maintainability drag with no regression. CARRIED-OPEN, ACCEPTED-DEFERRED; domain split scheduled for the next major touch.", where: "backend/aec/router.py" },
          { id: "FE-3", sev: "medium", title: "model/page.tsx god-component measured at 3,458 LOC (was 3,450) — growth essentially flat for the first time in several cycles, +0.23%. CARRIED-OPEN, ACCEPTED-DEFERRED pending the 5k-element benchmark trigger.", where: "frontend/app/project/[id]/model/page.tsx" },
          { id: "FE-1", sev: "low", title: "useScheduleFetch still calls pollUntilWarm<any>(...); the documented rationale is intact. CARRIED, unchanged, accepted debt — the fix is a union-friendly overload.", where: "frontend/app/hooks/useScheduleFetch.ts:123-126" },
          { id: "FE-2", sev: "low", title: "Legacy filter/sort/grouping schema-migration `any` types remain in the normalizer. CARRIED, unchanged, documented and accepted.", where: "frontend/app/lib/filters/normalize.ts:33,44,45,57,63,64,76" },
          { id: "FE-4", sev: "low", title: "A few hardcoded hex colors were never migrated onto design tokens. CARRIED, unchanged, accepted debt.", where: "frontend/app/components/MarkupPins.tsx:16-20,27 · ThemeToggle.tsx:46" },
          { id: "SEC-cloud-1", sev: "info", title: "EMEA region is inferred from URN convention; an unrecognized region returns None and is never guessed. Misrouting risk on EMEA calls is low-likelihood and currently unexercised (no EMEA hub onboarded). CARRIED, unchanged.", where: "backend/aps/cloud_ids.py:18-33,64-77" },
          { id: "FE-BASELINE-1", sev: "info", title: "Three raw-fetch call sites remain the only documented exceptions to the central apiFetch wrapper (a background blob sync, the admin plane's bearer-token client, and the viewer init). All deliberate and tested — re-confirmed unchanged this cycle. Tracked so future cycles stop re-flagging them.", where: "frontend/app/lib/api/persistence.ts · lib/adminApi.ts · lib/viewer/initViewer.ts:78" },
          { id: "CQ-WIZ-LEGACY-1", sev: "info", title: "planner.py documents two now-legacy plan-item types kept only so pre-rework persisted rows stay readable. Deliberately DEFERRED — deleting a compat path is a data-shape decision, not audit remediation. Precondition before removal: confirm no ProvisioningJob rows remain in `planning` status carrying those legacy item types. NOTE: the 08-04 report omits this item; it was verified still present on disk rather than dropped silently.", where: "backend/wizard/planner.py:31-33" }
        ],
        history: [
